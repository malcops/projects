...
for i in range(20):
    for j in range(16):
        # left/right products
        lr_prod = M[i][j] * M[i][j+1] * M[i][j+2] * M[i][j+3]
        if lr_prod > max_lr: max_lr = lr_prod
        # up/down products
        ud_prod = M[j][i] * M[j+1][i] * M[j+2][i] * M[j+3][i]
        if ud_prod > max_ud: max_ud = ud_prod
...
for i in range(17):
    for j in range(17):
        diag_prod = M[i][j] * M[i+1][j+1] * M[i+2][j+2] * M[i+3][j+3]
        if diag_prod > max_diag: max_diag = diag_prod
...
print("max_ud: {}".format(max_ud))
print("max_lr: {}".format(max_lr))
print("max_diag: {}".format(max_diag))
